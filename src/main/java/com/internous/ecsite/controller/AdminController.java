...
import com.internous.ecsite.model.form.SignupForm;
...
public class AdminController {
	...
	public String newSignup(SignupForm form, Model m) {
		m.addAttribute("userName", form.getUserName());
		m.addAttribute("password", form.getPassword());
	...
	public String addUser(SignupForm signupForm, LoginForm loginForm, Model m) {
		m.addAttribute("userName", loginForm.getUserName());
		m.addAttribute("password", loginForm.getPassword());
	...
	public boolean signup(@RequestBody SignupForm signupForm) {
		//TODO userテーブルに入力されたユーザ名が存在するかチェックする。
		int result = 0;
		result = userRepos.findCountByUserName(signupForm.getUserName());
		return result > 0;
	}
